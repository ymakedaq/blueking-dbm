...
from backend.flow.engine.bamboo.scene.mysql.dts.mysql_ha_to_ha_migrate import MysqlHaToHaMigrateFlow
...
class MySQLController(BaseController):
    ...
    def mysql_ha_to_ha_migrate_scene(self):
        flow = MysqlHaToHaMigrateFlow(root_id=self.root_id, data=self.ticket_data)
        flow.run_flow()
