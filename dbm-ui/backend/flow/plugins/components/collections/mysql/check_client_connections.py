...
from backend.flow.utils.mysql.check_client_connections import (
    check_client_connection,
    check_long_time_running_connection,
)
...
class CheckLongTimeRunningConnService(BaseService):
    """
    定义检测实例是否存储用户连接的活动节点（系统账号和内置账号会过滤）
    """
    ...
    def _execute(self, data, parent_data):
        ...
        results = check_long_time_running_connection(
            bk_cloud_id=kwargs["bk_cloud_id"], instances=kwargs["check_instances"]
        )
        process_infos = []
        for res in results:
            # 检查返回的每个实例的结果
...
class CCheckLongTimeRunningConnComponent(Component):
    name = __name__
    code = "check_long_time_running_connections"
    bound_service = CheckLongTimeRunningConnService
